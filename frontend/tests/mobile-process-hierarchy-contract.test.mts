...
test("mobile experiment form chooses draft for R&D and formal revision outputs for read-only roles",()=>{
  ...
  for(const marker of ["currentTestAssignment?.processRevisionId","api.task.getProcessRevision","api.task.getProcessArtifacts","api.report.downloadProcessArtifact","正式工艺 V","标准配方","研发版 SOP"]){assert.ok(source.includes(marker),`missing ${marker}`)}
  assert.ok(!source.includes("const latest=revisions[0]"),"tester must not drift to the latest revision");
